...
> **Status: live and reviewer-ready.** Public Streamlit client at **https://rag-aws-kb.streamlit.app** (Streamlit Community Cloud) signs in with Cognito JWT and calls a CDK-deployed AWS API that fronts a Bedrock Knowledge Base (S3 Vectors) and Anthropic Claude Haiku 4.5, with **per-session conversation memory** (AgentCore Memory + DynamoDB), **streaming responses** (Lambda Function URL + SSE), **runtime document upload + ingestion**, and **per-method APIGW throttling**. Smoke test passes 7/7; live multi-turn validation confirms DDB + AgentCore Memory state both stay in sync with the user's chat. The same stacks also serve a local dev client via [`./scripts/run_streamlit.sh`](scripts/run_streamlit.sh).

This repository is a productionization of a Streamlit + Gemini + FAISS prototype (preserved under [legacy/](legacy/)). It satisfies the AWS-Native Knowledge Base Agent take-home brief and lands six of the optional extensions (Cognito auth, AgentCore Runtime, AgentCore Memory, DynamoDB chat history, streaming responses, upload + ingestion endpoint), plus per-method APIGW throttling (partial cost-control) added in Phase 12 alongside the public Streamlit Cloud deployment.
...
| Agent runtime | **AgentCore Runtime + arm64 container** (Phase 8) | LangGraph; pure Lambda | This RAG is single-tool and stateless — pure Lambda would meet the brief. AgentCore Runtime was chosen specifically to **demonstrate the AgentCore primitives** the brief calls out as an acceptable orchestration layer, with the production-shaped pieces in place (managed runtime + Memory + console introspection + per-actor scoping) that a real multi-tool agent would use. Hybrid: REST `/query` is a thin Lambda proxy that calls `InvokeAgentRuntime`; the runtime container does the RAG. No LangGraph framework weight — pure boto3 inside `BedrockAgentCoreApp`. **Cost of the choice**: ~$0.20–0.50/day idle while the stack is up, plus the extra Lambda → AgentCore hop adds ~100–300ms to buffered `/query`. Acceptable for a demo + interview; for pure single-tool RAG in real production, pure Lambda is the cheaper choice. |
...
4. **`AwsCustomResource` + `SecretValue.unsafe_unwrap()` does NOT actually inject the secret value into Custom Resource calls** — CFN [explicitly does not resolve `secretsmanager` dynamic references](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/dynamic-references-secretsmanager.html) inside Custom Resource properties, so the literal `{{resolve:secretsmanager:ARN:SecretString:::}}` string was passed verbatim to `AdminSetUserPassword` as the password. This was the actual root cause behind the historical `NotAuthorizedException`-on-first-deploy trap; the legacy "rotate the secret to re-fire the CR" framing missed the real bug. **Fixed in Phase 11 hardening**: `AdminSetUserPassword` is now invoked by a small Lambda-backed Custom Resource that reads the secret via `boto3.client("secretsmanager").get_secret_value(...)` at runtime ([infra/stacks/auth_stack.py:113-175](infra/stacks/auth_stack.py#L113-L175)). The Custom Resource fires on every deploy where its properties diff — including a fresh deploy after `cdk destroy --all` (new secret ARN suffix → property diff → Lambda runs → password syncs). End-to-end verified by deliberately corrupting the live Cognito password, invoking the Lambda directly with a synthetic CFN event, and watching the smoke test pass 7/7 with no manual intervention.
...
- **APIGW method-level throttling** (Phase 12) — per-method rate/burst caps on every authenticated route (`/query` 10/20, `/ingest` 5/10 since it triggers KB embedding cost, others 10/20) + stage backstop 20/40. Replaces the API-key UsagePlan removed in Phase 8 without re-introducing static keys.

**Documented but not implemented** — production roadmap:
- **Per-user (per-`actor_id`) rate limits** + **APIGW WAF** (rate, geo, OWASP common rules). Current throttling is account-wide per-method, not per-caller.
...
- **Consolidate the `agent/rag.py` ↔ `lambda_stream/stream_app.py` duplication.** The system prompt, INSUFFICIENT_CONTEXT marker handling, confidence formula, and AgentCore Memory + DDB write logic exist in both files because Docker contexts are isolated and AgentCore Runtime's `InvokeAgentRuntime` API is buffered-only as of May 2026. Any change to one path must be mirrored to the other — a real correctness risk worth a CI lint that fails on divergence. If/when AgentCore Runtime exposes a streaming counterpart, both paths can collapse into the runtime and the duplication disappears.
...
**Test-user password sync** is now automatic — the `AdminSetUserPassword` custom resource re-fires on every fresh deploy via its ARN-bound `physical_resource_id` (Phase 11 hardening, see [§7](#7-gotchas-from-the-live-deploy) #4). The smoke test should pass directly. If for any reason the first auth fails with `NotAuthorizedException`, the historical one-liner stop-gap is still available:
...
### 10.6 Production-like deployment (Streamlit Community Cloud)

Phase 12 — the same 4 CDK stacks back a publicly reachable Streamlit app at **https://rag-aws-kb.streamlit.app**, hosted free on Streamlit Community Cloud. No additional AWS infra is provisioned for the client (AWS App Runner stopped accepting new customers as of 2026-04-30; Streamlit Cloud is the zero-overhead path for a Streamlit-shaped app).

The CDK is already prod-shaped:
- Cognito App Client registers **both** `http://localhost:8501/oauth2callback` and `https://rag-aws-kb.streamlit.app/oauth2callback` as callback URLs, so the same User Pool serves both local dev (via `./scripts/run_streamlit.sh`) and Streamlit Cloud (via dashboard secrets).
- APIGW preflight + Function URL CORS allow both origins.
- Method-level throttling (see [§9](#9-production-hardening)) caps abuse.
- `RemovalPolicy.DESTROY` is retained intentionally — you tear down on demand. Note: every `cdk destroy --all` + redeploy rotates Cognito IDs and the App Client secret, so you must re-paste secrets into Streamlit Cloud after each redeploy (see step 3 below).

**One-time setup** (after `cdk deploy --all` and a public GitHub push):

1. **Connect the repo to Streamlit Cloud**: go to https://share.streamlit.io, click "New app", point at your GitHub repo + branch, set:
   - Main file path: `streamlit_client/app.py`
   - Python version: 3.12
   - (Streamlit Cloud auto-detects `streamlit_client/requirements.txt`.)
2. **Reserve the subdomain**: in **App settings → General → URL**, set the custom subdomain to `rag-aws-kb` (so the public URL is `https://rag-aws-kb.streamlit.app`). This must match the Cognito callback URL above exactly.
3. **Paste secrets**: locally run the helper and paste the output into **App settings → Secrets**:
   ```bash
   python scripts/print_streamlit_cloud_secrets.py | pbcopy
   # then paste in the Streamlit Cloud Secrets editor → Save
   ```
   The script reads live CFN outputs + Secrets Manager and writes a TOML block with `redirect_uri=https://rag-aws-kb.streamlit.app/oauth2callback` plus all the Cognito + API + stream URLs. The `cookie_secret` is persisted locally in `.streamlit_cloud_cookie_secret` (gitignored) so subsequent re-pastes keep live sessions warm.
...
**Browser flow** is identical to [§10.4.1](#1041-what-a-user-sees-browser-walkthrough) but the Cognito Hosted UI redirects back to `https://rag-aws-kb.streamlit.app/oauth2callback` instead of localhost.

**What's NOT in the prod posture** (consciously deferred):
- No custom DNS — Streamlit Cloud free tier only supports `*.streamlit.app`.
- No CloudWatch budget alarm — manual monitoring; cumulative spend across 12 phases is still under $3.
- No per-user (`actor_id`) rate limits, no WAF, no VPC, no KMS CMKs, no CI/CD, no multi-account/stage separation. All listed under [§9](#9-production-hardening) production roadmap.
...
| [streamlit_client/](streamlit_client/) | Streamlit client (chat + sidebar + uploader). Same code path serves local dev and the public Streamlit Cloud app at https://rag-aws-kb.streamlit.app. |
| [scripts/](scripts/) | `upload_docs.py`, `start_ingestion.py`, `smoke_test.py`, `get_id_token.py`, `run_streamlit.sh` (local launcher, auto-syncs `secrets.toml`), `print_streamlit_cloud_secrets.py` (Phase 12 — prints TOML for paste into Streamlit Cloud Secrets editor). |
...
- **Test-user password sync** — automatic on every deploy via a Lambda-backed Custom Resource (Phase 11 fix, see [§7](#7-gotchas-from-the-live-deploy) #4). The legacy `admin-set-user-password` one-liner is preserved as a stop-gap only.
- **Streamlit Cloud secrets are dashboard-managed** — after every `cdk destroy --all` + redeploy, re-paste from `scripts/print_streamlit_cloud_secrets.py` (no equivalent of the local `run_streamlit.sh` auto-sync). See [§10.6](#106-production-like-deployment-streamlit-community-cloud).
