...
**Test-user password sync** is now automatic — the `AdminSetUserPassword` custom resource re-fires on every fresh deploy via its ARN-bound `physical_resource_id` (Phase 10 hardening, see [§7](#7-gotchas-from-the-live-deploy) #4). The smoke test should pass directly. If for any reason the first auth fails with `NotAuthorizedException`, the historical one-liner stop-gap is still available:
